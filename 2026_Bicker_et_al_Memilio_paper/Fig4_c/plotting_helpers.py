...
def posterior_predictive_plot(
    ...
    **plot_kwargs,
    ...
    for e, k in enumerate(inds):
        ...
        if sim.ndim != 2:
            raise ValueError("Simulator output must be 2D (states×T) or (T×states)")
        if sim.shape[1] == T:
            sim_st_t = sim
        elif sim.shape[0] == T:
            sim_st_t = sim.T
        else:
            raise ValueError("Simulator output must be (states×T) or (T×states) with T=len(obs_t)")
    ...
    if median_pos is not None:
        ax.plot(obs_t, qtraj[median_pos, :], color=median_color, lw=median_lw, label="Simulation")
    ...
    ax.set_xticks(ticks)
    ax.set_xticklabels(ticklabels.strftime("%Y-%m-%d"), rotation=35, ha="right")
