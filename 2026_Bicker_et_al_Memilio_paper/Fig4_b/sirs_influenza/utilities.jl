...
- `burn_in`: number of initial iterations per chain to discard.
...
function posterior_predictive_plot(
    ...
    # Array(chain) has size (iterations, parameters, chains)
    arr = chain.value.data[:,2:end,:] # be aware lp is expected to be the first column here!
    n_iter, n_param, n_chain = size(arr)

    @assert 0 ≤ burn_in < n_iter "burn_in must be between 0 and n_iter-1"
    first_it = burn_in + 1
    ...
    # Parameter names for NamedTuple
    param_names = MCMCChains.names(chain, :parameters)

    # Sample draw indices (uniformly over all chains and post-burn-in iterations)
    # Linear index in 1:n_kept maps to (iteration, chain)
    inds = sample(rng, 1:n_kept, n_draw; replace = false)

    idx_to_it_ch(k) = begin
        it = ((k - 1) % n_it_kept) + first_it
        ch = Int((k - 1) ÷ n_it_kept) + 1
        return it, ch
    end
    ...
    for (e, k) in enumerate(inds)
        ...
        # parameter vector for this draw
        θ_vec = arr[it, :, ch]               # Vector{Float64} of length n_param
        params = (; zip(param_names, θ_vec)...)  # NamedTuple, e.g. params.β, params.σ, ...
    ...
    for i in 1:floor(Int, nq / 2)
        ...
        plot!(plt, obs_t, mid; ribbon = width, fillalpha = 0.2, label = label)
    end
