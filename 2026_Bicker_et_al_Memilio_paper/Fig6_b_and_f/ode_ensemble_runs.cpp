...
mio::IOResult<void> set_contact_matrices(std::string data_dir, mio::osecir::Parameters<ScalarType>& params)
{
    ...
    for (auto&& contact_location : params::contact_locations) {
        BOOST_OUTCOME_TRY(auto&& baseline,
                          mio::read_mobility_plain(mio::path_join(data_dir, "Germany", "contacts",
                                                                  ("baseline_" + contact_location.second + ".txt"))));
        ...
        contact_matrices[size_t(contact_location.first)].get_minimum() =
            Eigen::Matrix<ScalarType, Eigen::Dynamic, Eigen::Dynamic>::Zero(num_groups, num_groups);
    }
    params.template get<mio::osecir::ContactPatterns<ScalarType>>() =
...
mio::osecir::Parameters<ScalarType> initialize_osecir_params(std::string data_dir)
{
    ...
    for (size_t group = 0; group < params::num_groups; group++) {
        ...
        // Mean stay times
        parameters.get<mio::osecir::TimeExposed<ScalarType>>()[mio::AgeGroup(group)] =
            uncertain(params::timeExposed[group]);
        ...
        parameters.get<mio::osecir::TimeInfectedSevere<ScalarType>>()[mio::AgeGroup(group)] =
            uncertain(params::timeInfectedSevere[group]);
        ...
        parameters.get<mio::osecir::DeathsPerCritical<ScalarType>>()[mio::AgeGroup(group)] =
            uncertain(params::deathsPerCritical[group]);
    }
...
mio::IOResult<mio::Graph<Model, mio::MobilityParameters<ScalarType>>> get_graph(std::string data_dir)
{
    ...
    const auto& set_node_function =
        mio::set_nodes<ScalarType, mio::osecir::TestAndTraceCapacity<ScalarType>,
                       mio::osecir::ContactPatterns<ScalarType>, mio::osecir::Model<ScalarType>,
                       mio::MobilityParameters<ScalarType>, mio::osecir::Parameters<ScalarType>,
                       decltype(read_function_nodes), decltype(node_id_function)>;
    const auto& set_edge_function =
        mio::set_edges<ScalarType, params::ContactLocation, mio::osecir::Model<ScalarType>,
                       mio::MobilityParameters<ScalarType>, mio::MobilityCoefficientGroup<ScalarType>,
                       mio::osecir::InfectionState, decltype(read_function_edges)>;

    auto pydata_path          = mio::path_join(data_dir, "Germany", "pydata");
    auto population_data_path = mio::path_join(pydata_path, "county_current_population.json");

    BOOST_OUTCOME_TRY(set_node_function(model_params, params::start_date, end_date, pydata_path, population_data_path,
                                        true, params_graph, read_function_nodes, node_id_function,
                                        scaling_factor_infected, scaling_factor_icu, tnt_capacity_factor, 0, false,
                                        true));
    BOOST_OUTCOME_TRY(set_edge_function(mio::path_join(data_dir, "Germany", "mobility", "commuter_mobility_2019.txt"),
                                        params_graph, mobile_compartments, params::contact_locations.size(),
