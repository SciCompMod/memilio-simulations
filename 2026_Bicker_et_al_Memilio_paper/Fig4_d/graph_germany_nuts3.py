...
def plot_aggregated_over_regions(
    ...
    ax.plot(x, agg_median, lw=3,
        label= "Aggregated simulation", color=color, zorder=1)
...
def plot_icu_on_germany(simulations):
    ...
    population = pd.read_json(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'build/_deps/memilio-src/data/Germany/pydata/county_current_population.json'))
    ...
    plot_map(values[0], map_data, axes[0], "Median ICU", vmin, vmax)
    plot_map(values[-1], map_data, axes[1], "Median ICU", vmin, vmax)
...
def run_germany_nuts3_simulation(scaling_factor, damping_values, t_E, t_ISy, t_ISev, t_Cr, mu_CR, mu_IH, mu_HU, mu_UD, transmission_prob, export_timeseries=False):
    ...
    sim = Simulation(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'build/_deps/memilio-src/data'),
